...
interface FilmsDataSource {
    ...
    suspend fun getFilmDetailed(id: Int): Result<FilmDetailed, Exception>
    suspend fun saveFilms(films: List<Film>)
    suspend fun saveFilm(film: FilmDetailed)
    suspend fun saveFilm(film: Film)
    suspend fun changeLikeState(id: Int, newState: Boolean)
    suspend fun checkLiked(id: Int): Boolean
}
